export { Header } from './Header';
export { Hero } from './Hero';
export { HeroSection } from './HeroSection';
export { ServicesSection } from './ServicesSection';
export { ExperienceSection } from './ExperienceSection';
export { PlatformSection } from './PlatformSection';
export { SegmentsSection } from './SegmentsSection';
export { TestimonialsSection } from './TestimonialsSection';
export { Footer } from './Footer';

// Export all types
export type { HeaderProps, Logo, NavigationItem, CTAButton } from './Header';
export type { HeroProps, CTALink, Stat } from './Hero';
export type { HeroSectionProps, CTAButton as HeroCTAButton, Stat as HeroStat } from './HeroSection';
export type { ServicesSectionProps, Service, ServiceLink, CTABanner } from './ServicesSection';
export type { ExperienceSectionProps, Feature, Image, MiniStat } from './ExperienceSection';
export type { PlatformSectionProps, PlatformFeature } from './PlatformSection';
export type { SegmentsSectionProps, Segment } from './SegmentsSection';
export type { TestimonialsSectionProps, Testimonial, ClientLogo } from './TestimonialsSection';
export type {
  FooterProps,
  FooterLogo,
  FooterLink,
  FooterSection,
  SocialLink,
  LegalLink,
} from './Footer';
